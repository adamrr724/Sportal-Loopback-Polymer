...
function expandDeprecated(context, done) {
    ...
    if (browsers.length > 0) {
        ...
        for (const browser of browsers) {
            ...
            const plugin = browser.platform || name.indexOf('/') !== -1 ? 'sauce' : 'local';
...
function validate(options, done) {
    if (options['webRunner']) {
        return done('webRunner is no longer a supported configuration option. ' +
            'Please list the files you wish to test as arguments, ' +
            'or as `suites` in a configuration object.');
    }
    if (options['component']) {
        return done('component is no longer a supported configuration option. ' +
            'Please list the files you wish to test as arguments, ' +
            'or as `suites` in a configuration object.');
    }
